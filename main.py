...
async def rmove_user_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Do NOT remove the user from 'removed_users' list here
        # This allows the user to be removed later via /list_rmoved_rmove
        # removed = remove_user_from_removed_users(group_id, target_user_id)
        removed = False  # Keep the user in the list
        if removed:
        ...
        message = escape_markdown("⚠️ Failed to remove user from 'Removed Users' list. Please try again later.", version=2)
        await context.bot.send_message(
            chat_id=user.id,
            text=message,
            parse_mode='MarkdownV2'
        )
        logger.error(f"Error removing user {target_user_id} from 'Removed Users' for group {group_id} by user {user.id}: {e}")
        return
